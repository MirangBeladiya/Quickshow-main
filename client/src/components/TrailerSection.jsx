  ...
  return (
    <div className="px-6 md:px-16 lg:px-24 xl:px-44 py-20 overflow-hidden">
      ...
      <div className="relative mt-6">
        <BlurCircle top="-90px" right="-100px"/>
        <ReactPlayer url={currentTrailer.videoUrl} controls={false} className="mx-auto max-w-full" width="960px" height="540px"/>
        <BlurCircle top="320px" left="10px"/>
      </div>
      <div className="group grid grid-cols-4 max-md:grid-cols-2 gap-4 md:gap-8 mt-8 max-w-3xl mx-auto">
        {dummyTrailers.map((trailer) => (
            <div key={trailer.image} onClick={() => setCurrentTrailer(trailer)} className="relative group-hover:not-hover:opacity-50 hover:-translate-y-1 duration-300 transition max-md:h-60 md:max-h-60 cursor-pointer">
                ...
                <PlayCircleIcon className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-5 md:w-8 h-5 md:h-12 transform" strokeWidth={1.6}/>
            </div>
        ))}
      </div>
    </div>
